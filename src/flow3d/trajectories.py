...
def get_wander_w2cs(ref_w2c, lookat: torch.Tensor, focal_length, num_frames, max_disp: float = 48.0, **_):
    device = ref_w2c.device
    c2w = torch.linalg.inv(ref_w2c)
    z = torch.linalg.norm(c2w[:3, 3] - lookat).item()
    c2w = c2w.detach().cpu().numpy()

    max_trans = max_disp * z / focal_length
    output_poses = []
