...
def init_fg_from_tracks_3d(
    cano_t: int, tracks_3d: TrackObservations, motion_coefs: torch.Tensor, init_scale: float = 1.0,
    ...
    if color_activation == "sh_relu":
        colors = RGB2SH(tracks_3d.colors)
    else:
    ...
    scales = torch.log(scales.repeat(1, 3) * init_scale)
    ...
    gaussians = GaussianParams(means, quats, scales, colors, opacities, motion_coefs, color_activation=color_activation)
    return gaussians
...
def init_motion_params_with_procrustes(
    ...
    valid_mask = valid_mask & (tracks_3d.visibles.sum(dim=1) > 0.10 * num_frames)
    ...
    #                 server.scene.add_point_cloud("points", pts[:, t], clrs, point_size=0.01)
    #                 time.sleep(0.3)
    ...
    ids = ids[counts > 100]
    num_bases = len(ids)
    sampled_centers = sampled_centers[:, ids.cpu()]
...
def run_initial_optim(
    ...
    optimizer = torch.optim.Adam(
        [
            ...
            {"params": bases.params["transls"], "lr": 3e-2 * scene_scale},
            {"params": fg.params["motion_coefs"], "lr": 1e-2},
            {"params": fg.params["means"], "lr": 1e-3 * scene_scale},
        ],
    )
    ...
    for i in pbar:
        ...
        loss += track_3d_loss * 1.0 / scene_scale
        ...
        loss += small_acc_loss * w_smooth / scene_scale

        small_acc_loss_tracks = compute_accel_loss(positions)
        loss += small_acc_loss_tracks * w_smooth * 0.5 / scene_scale
