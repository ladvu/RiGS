...
class GaussianParams(nn.Module):
    def __init__(
        ...
        params_dict = {
            ...
            "colors": nn.Parameter(colors), # sh0
        ...
        self.color_activation = (lambda x: torch.clamp_min(SH2RGB(x), 0.0)) if color_activation == "sh_relu" else torch.sigmoid
...
def se3_interpolate(T1, T2, s):
    # batched
    assert len(T1.shape) == 3 and len(T1.shape) == 3
    if T1.shape[1] == 3:
        T1 = torch.cat([T1, torch.tensor([0,0,0,1], device=T1.device, dtype=T1.dtype).reshape(1, 1, 4).repeat(T1.shape[0],1,1)], dim=1)
    if T2.shape[1] == 3:
        T2 = torch.cat([T2, torch.tensor([0,0,0,1], device=T2.device, dtype=T2.dtype).reshape(1, 1, 4).repeat(T2.shape[0],1,1)], dim=1)
    T1_se3 = SE3.from_matrix(T1, normalize=True)
    T2_se3 = SE3.from_matrix(T2, normalize=True)
    delta = T1_se3.inv().dot(T2_se3)
    T_interp = T1_se3.dot(SE3.exp(s.unsqueeze(1) * delta.log()))
    return T_interp.as_matrix()[:, :3, :]
...
class MotionBases(nn.Module):
    ...
    def compute_transforms_to_center(self, center: torch.Tensor, coefs: torch.Tensor) -> torch.Tensor:
        center = center.long()
        transls = self.params["transls"][:, center]  # (K, G, 3)
        rots = self.params["rots"][:, center]  # (K, G, 6)
        transls = torch.einsum("pk,kpi->pi", coefs, transls) #(G, 3)
        rots = torch.einsum("pk,kpi->pi", coefs, rots)  # (G, 6)
        rotmats = cont_6d_to_rmat(rots)  # (G, 3, 3)
        return torch.cat([rotmats, transls[..., None]], dim=-1) # (G, 3, 4)
    
    def compute_transforms_continuous(self, ts: torch.Tensor, coefs: torch.Tensor) -> torch.Tensor:
        """
        :param ts (G)
        :param coefs (G, K)
        returns transforms (G, 3, 4)
        """
        ts_floor = torch.clamp(torch.floor(ts).long(), 0, self.num_frames - 1)
        ts_ceil = torch.clamp(torch.ceil(ts_floor).long(), 0, self.num_frames - 1)
        transform_floor = self.compute_transforms_to_center(ts_floor, coefs)
        transform_ceil = self.compute_transforms_to_center(ts_ceil, coefs)
        s = ts - ts_floor.float()
        transform = se3_interpolate(transform_floor, transform_ceil, s)
        return transform

    
    def compute_transforms_rel(self, from_ts: torch.Tensor, to_ts: torch.Tensor, means: torch.Tensor, coefs: torch.Tensor) -> torch.Tensor:
        """
        Args:
        :param from_ts (G, )
        :param to_ts (B)
        ...
        returns transforms (G, B, 3, 4)
        """
        transform_from = self.compute_transforms_continuous(from_ts, coefs)  # (G, 3, 4)
        r = transform_from[:, :3, :3]  # (G, 3, 3)
        t = transform_from[:, :3, 3]  # (G, 3)
        r_inv = r.transpose(-1, -2)  # (G, 3, 3)
        means_canonical = torch.einsum(
            "gij,gj->gi", r_inv, means - t
        )  # (G, 3)

        transform_to = self.compute_transforms(to_ts, coefs)  # (G, B, 3, 4)
        means_to = torch.einsum(
            "gbij,gi->gbj", transform_to, F.pad(means_canonical, (0, 1), dim=-1, value=1.0)
        )  # (G, B, 3)
        return means_to
