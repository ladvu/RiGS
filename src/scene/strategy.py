# This will contain the strategy for dynamic gaussians
...
from utils.gs_utils import make_transient
...
class DynamicStrategy(Strategy):
    ...
    make_transient_every: int = 1000
    make_transient_start_iter:int = 7000
    ...
    def step_post_backward(
        ...
        if (
            ...
            # prune GSs
            n_prune = self._prune_gs(params, optimizers, state, preset_mask, step)
            if self.verbose:
                print(
    ...
    def _prune_gs(
        ...
        optimizers: Dict[str, torch.optim.Optimizer],
        state: Dict[str, Any],
        preset_mask: torch.Tensor,
        step: int,
    ) -> int:
        is_prune = torch.sigmoid(params["opacities"].flatten()) < self.prune_opa
        if step > self.make_transient_start_iter and step % self.make_transient_every == 0:
            assert preset_mask is not None, "preset_mask is required for making transient gaussians."
            is_prune = is_prune | preset_mask
