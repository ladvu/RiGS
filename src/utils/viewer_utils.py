...
class Keyframe:
    ...
    @staticmethod
    def from_camera(camera: viser.CameraHandle, aspect: float, timestamp: float = 0.0) -> Keyframe:
        return Keyframe(
            camera.position,
            camera.wxyz,
...
def populate_general_render_tab(
    ...
    with folder:
        ...
        def _(event: viser.GuiEvent) -> None:
            ...
            # Get current timestamp from render_tab_state
            current_timestamp = getattr(render_tab_state, 'timestamp', 0.0)
        ...
        rotate_up_slider = server.gui.add_slider(
            "Roll Angle (deg)",
            min=0.0,
            max=360.0,
            initial_value=0.0,
            step=1.0,
            hint="Rotate camera up direction around the lookat axis (camera roll)",
        )

        @rotate_up_slider.on_update
        def _(event: viser.GuiEvent) -> None:
            assert event.client is not None
            # Convert slider value from degrees to radians
            theta_rad = rotate_up_slider.value * np.pi / 180.0
            
            # Get the forward direction (lookat direction) of the camera
            # In camera coordinate system, forward is -z direction
            camera_rotation = tf.SO3(event.client.camera.wxyz)
            forward_direction = camera_rotation @ np.array([0.0, 0.0, -1.0])
            
            # Create rotation around the forward direction (lookat axis)
            # Use Rodrigues' rotation formula via axis-angle representation
            axis_angle = forward_direction * theta_rad
            forward_rotation = tf.SO3.exp(axis_angle)
            
            # Apply rotation to the base up direction
            base_up = camera_rotation @ np.array([0.0, -1.0, 0.0])
            event.client.camera.up_direction = forward_rotation @ base_up
    ...
    camera_path = CameraPath(server, duration_number, time_enabled=time_enabled)
    camera_path.tension = tension_slider.value
